...
@author: xiaosonh, Alice
change: add some func
'''
import os
import sys
import argparse
...
class Labelme2YOLO(object):
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        if not os.path.exists(img_path):
            if self.local_image_dir == None and json_data['imageData'] != None:

                img = utils.img_b64_to_arr(json_data['imageData'])
                PIL.Image.fromarray(img).save(img_path)
            
            if self.local_image_dir != None:

                shutil.copy(os.path.join(self.local_image_dir, img_name), img_path)
            
            if self.local_image_dir == None and json_data['imageData'] == None:

                print('your Datasets Setting is wrong!!!! Please check it')
        print(img_path)
...
def arg_parser_use():
    ...
    parser.add_argument('--local_image_dir', type=str,default=None,
                        help='Please input the path of the labelme image files.')
    ...
    convertor = Labelme2YOLO(args.json_dir, to_seg=args.seg, local_image_dir=args.local_image_dir)
...
'''
    The output file will save in the same folder, 
    then subfolder named YOLODataset
'''
